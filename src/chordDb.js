...
export function lookupChord(name) {
  ...
  // 3. Extraer la parte anglo de "Latina (Anglo)"
...
export function enrichNotesWithChordDb(notes) {
  ...
  return notes.map(note => {
    ...
    const chord = lookupChord(chordKey);
...
export function enrichSongData(songData) {
  ...
  if (songData.notes && songData.notes.length > 0) {
    ...
    for (const note of songData.notes) {
      ...
      const chord = lookupChord(key);
      if (!chord) continue;
      
      if (!seenChords.has(chord.anglo)) {
        seenChords.add(chord.anglo);
